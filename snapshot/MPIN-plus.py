...
def build_GNN(in_channels, out_channels, k, base):
    if base == "GAT":
        gnn = DynamicGAT(in_channels=in_channels, out_channels=out_channels, k=k).to(
            device
        )
    elif base == "GCN":
        gnn = DynamicGCN(in_channels=in_channels, out_channels=out_channels, k=k).to(
            device
        )
    elif base == "SAGE":
...
def build_GNN_static(in_channels, out_channels, k, base):
    if base == "GAT":
        gnn = StaticGAT(in_channels=in_channels, out_channels=out_channels, k=k).to(
            device
        )
    elif base == "GCN":
        gnn = StaticGCN(in_channels=in_channels, out_channels=out_channels, k=k).to(
            device
        )
    elif base == "SAGE":
        gnn = StaticGraphSAGE(
        ...
        gnn = StaticGraphSAGEPlusDA(
            in_channels=in_channels, out_channels=out_channels, k=k
        ).to(device)
    elif base == "SAGE++DAC":
        gnn = StaticGraphSAGEPlusDAC(
            in_channels=in_channels, out_channels=out_channels, k=k
        ).to(device)
    elif base == "SAGE++DAMC":
...
def get_window_data(start, end, ratio):
    ...
    X_mask = base_X_mask[
        int(len(base_X) * start * ratio) : int(len(base_X) * end * ratio)
    ]
    return X, X_mask


def window_imputation(
    start,
    end,
    sample_ratio,
    ...
    if args.dynamic == "true":
        ...
        gnn = build_GNN(
            in_channels=in_channels, out_channels=out_channels, k=args.k, base=args.base
        )
        gnn2 = build_GNN(
            in_channels=in_channels, out_channels=out_channels, k=args.k, base=args.base
        )
        ...
        gnn = build_GNN_static(
            in_channels=in_channels, out_channels=out_channels, k=args.k, base=args.base
        )
        gnn2 = build_GNN_static(
            in_channels=in_channels, out_channels=out_channels, k=args.k, base=args.base
        )
    ...
    for pre_epoch in range(epochs):
        ...
        for i in range(graph_impute_layers):
            ...
            temp_loss = torch.sum(torch.abs(X - pred) * X_mask) / (
                torch.sum(X_mask) + 1e-5
            )
        ...
        with torch.no_grad():
            eval_impute_error = torch.sum(
                torch.abs(trans_X - trans_eval_X) * eval_mask
            ) / torch.sum(eval_mask)
            ...
            eval_impute_error_mse = cal_mse(
                trans_X, trans_eval_X, eval_mask
            ...
            arg_min_mae_error, min_mae_error = min(
...
if args.dataset in ["KDM", "WDS", "LHS"]:
    base_X = load_WiFi_dataset(
        window=args.window, dataset_name=args.dataset, time_step=5, method="mpin"
    )
    base_X_mask = (~np.isnan(base_X)).astype(int)
    # base_X = StandardScaler().fit_transform(base_X)
    # Đảm bảo base_X có thể ghi trước khi điền giá trị khuyết
    base_X = base_X.copy()
    ...
    print(
        "base WiFi {datasetname} data shape:".format(datasetname=args.dataset),
        base_X.shape,
        base_X_mask.shape,
    )
    ...
    base_X = load_airquality_dataset(
...
incre_mode = (
    args.incre_mode
)  # 'alone',  'data', 'state', 'state+transfer', 'data+state', 'data+state+transfer'
...
for iteration in range(num_of_iteration):
    ...
    for w in range(num_windows):
        ...
        if w == 0:
            mask_last, window_results = window_imputation(
                start=w, end=w + 1, sample_ratio=1
            )
...
# Add Pearson suffix to filename if enabled
pearson_suffix = f"_pearson_{args.delta}" if args.use_pearson == "true" else ""

if args.prefix == "testNumStream":
    avg_df.to_csv(
        f"./exp_results/{args.prefix}_{args.dataset}_{args.k}_{args.base}_incre_{args.incre_mode}_window_{args.window}_epoch_{args.epochs}_eval_{args.eval_ratio}_stream_{args.stream}{pearson_suffix}.csv",
        header=True,
        index=True,
    )
else:
    avg_df.to_csv(
        f"./exp_results/{args.prefix}_{args.dataset}_{args.k}_{args.base}_incre_{args.incre_mode}_window_{args.window}_epoch_{args.epochs}_eval_{args.eval_ratio}{pearson_suffix}.csv",
        header=True,
        index=True,
    )
print("done!")
