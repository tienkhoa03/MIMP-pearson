"""Aggregate CSV result files under continuous/iif_exp/Airquality into one CSV.

Usage:
  python aggregate_airquality.py [--src DIR] [--out FILE]

Defaults:
  src: continuous/iif_exp/Airquality
  out: continuous/plots/airquality_aggregated.csv
"""
import argparse
import os
import glob
...
def main():
    ...
    p.add_argument("--src", default=os.path.join("iif_exp", "Airquality"))
    p.add_argument("--out", default=os.path.join("plots", "airquality_aggregated.csv"))
    args = p.parse_args()
