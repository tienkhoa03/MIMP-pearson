...
def main():
    ...
    folder = os.path.join(base, 'ICU')
    dataset = 'ICU'
    streams = [0.001, 0.01, 0.1, 1.0]
    metrics = collect_dataset_metrics(folder, dataset, streams)

    xlabels = [str(int(s * 100)) if s >= 0.01 else str(s * 100) for s in streams]
    # format: ['0.1','1','10','100']
    xlabels = [str(float(s) if '.' in str(s) else s) for s in xlabels]
    ...
    out_mre = os.path.join(folder, 'ICU_MRE.png')
    out_time = os.path.join(folder, 'ICU_Time.png')
